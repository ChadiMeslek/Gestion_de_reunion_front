...
const AddPersonneToReunion = () => {
    ...
    const addPersonneToReunion = async (e) => {
        e.preventDefault();
        await axios.put(`http://localhost:8080/reunions/${idReunion}/personnes/${personneId}`);
        navigate(`/viewReunion/${idReunion}`);
    };

    return (
        <div className='col-sm8 py-3 px-5 border border-2 shadow' style={{ marginTop: '25px' }}>
            <h2 className='text-center'>Ajouter une Personne à la Réunion</h2>
            {reunion && (
                <>
                    <h4 className='text-center mb-4'>{`Titre: ${reunion.titre}`}</h4>
                    <form onSubmit={(e) => addPersonneToReunion(e)}>
...
export default AddPersonneToReunion;
